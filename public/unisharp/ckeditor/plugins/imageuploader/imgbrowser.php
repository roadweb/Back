...
<?php
    ...
    if(file_exists($useruploadpath)){
        ...
        $fi = new FilesystemIterator($useruploadpath, FilesystemIterator::SKIP_DOTS);
        $count = iterator_count($fi);
        if ($count == 1) { echo "<p class=\"folderInfo\">In total: $count File</p>"; }
        else { echo "<p class=\"folderInfo\">In total: $count Files - <span id=\"finalsize\"></span></p>"; };
...
<script>$( '#finalsize' ).html('<?php echo $filesizefinal; ?> KB');</script>
